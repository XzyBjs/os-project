...
// handle functions
int handle_f(tcp_buffer *wb,char *args, int len, int auid) {
...
int handle_mk(tcp_buffer *wb,char *args, int len, int auid) {
...
int handle_mkdir(tcp_buffer *wb, char *args, int len, int auid) {
    char *name=strtok(args, " \r\n");
    if (name == NULL) {
...
int handle_rm(tcp_buffer* wb, char *args, int len, int auid) {
    char *name=strtok(args, " \r\n");
    if (name == NULL) {
        char err[]="Missing file name";
        Error("Missing file name");
...
int handle_cd(tcp_buffer *wb, char *args, int len, int auid) {
    char *name=strtok(args, " \r\n");
    if (name == NULL) {
        char err[]="Missing dir name";
        Error("Missing dir name");
...
int handle_rmdir(tcp_buffer *wb, char *args, int len, int auid) {
    char *name=strtok(args, " \r\n");
    if (name == NULL) {
        char err[]="Missing dir name";
        Error("Missing dir name");
...
int handle_ls(tcp_buffer *wb, char *args, int len, int auid) {
    entry *entries = NULL;
    int n = 0;
...
int handle_cat(tcp_buffer *wb, char *args, int lenth, int auid) {
    char *name=strtok(args, " \r\n");
    if (name == NULL) {
...
int handle_w(tcp_buffer *wb, char *args, int lenth, int auid) {
    char *name;
    uint len;
    char *data;
    name = strtok(args, " \r\n");
...
int handle_i(tcp_buffer *wb, char *args, int lenth, int auid) {
    char *name;
    uint pos;
    uint len;
    char *data;
...
int handle_d(tcp_buffer *wb, char *args, int lenth, int auid) {
    char *name;
    uint pos;
    uint len;
    name=strtok(args, " \r\n");
...
int handle_e(tcp_buffer *wb, char *args, int lenth, int auid) {
    char msg[] = "Bye!";
    reply(wb, msg, strlen(msg) + 1);
    Log("Exit");
    initialize_users_vector(); // reset user vector
    g_auid = -1; // reset current user ID
    return -1;
}

int handle_login(tcp_buffer *wb, char *args, int lenth, int auid) {
...
int handle_logout(tcp_buffer *wb, char *args, int lenth, int auid) {
...
static struct {
    ...
    int (*handler)(tcp_buffer *wb, char *, int, int);
                 ...
                 {"login", handle_login}, {"logout", handle_logout}};
...
int on_recv(int id, tcp_buffer *wb, char *msg, int len) {
    ...
    for (int i = 0; i < NCMD; i++)
        if (p && strcmp(p, cmd_table[i].name) == 0) {
            ret = cmd_table[i].handler(wb, p + strlen(p) + 1, len - strlen(p) - 1, g_auid);
            break;
        }
    if (ret == 1) {
